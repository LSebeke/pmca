...
def test_get_tools_includes_manage_rag_retention_when_store_has_content():
    cfg = _config()
    store = _store_with_chunks("fn `foo`")
    tools = get_tools(cfg, store)
    assert tools is not None
    names = {t["function"]["name"] for t in tools}
    assert "manage_rag_retention" in names


def test_get_tools_excludes_manage_rag_retention_when_store_is_empty():
    cfg = _config()
    tools = get_tools(cfg, _empty_store())
    assert tools is None  # no tools at all
...
def _store_with_named_chunks(*specs: tuple[str, str]) -> VectorStore:
    """specs: [(source_file_str, label), ...]"""
    store = MagicMock(spec=VectorStore)
    chunks = [
        Chunk(content=f"content of {label}", source_file=Path(src), label=label)
        for src, label in specs
    ]
    store._chunks = chunks
    return store


def test_retain_adds_chunk_and_returns_summary():
    store = _store_with_named_chunks(("/src/a.py", "class Foo"))
    cfg = _config()
    retained: list = []
    result = execute_manage_rag_retention(
        {"retain": [{"file": "/src/a.py", "label": "class Foo"}]},
        cfg, store, retained,
    )
    assert len(retained) == 1
    assert retained[0].label == "class Foo"
    assert "Retained 1 new chunk" in result
    assert "[Retained RAG: 1 chunk" in result


def test_retain_is_idempotent():
    store = _store_with_named_chunks(("/src/a.py", "class Foo"))
    cfg = _config()
    retained: list = [Chunk(content="content of class Foo", source_file=Path("/src/a.py"), label="class Foo")]
    result = execute_manage_rag_retention(
        {"retain": [{"file": "/src/a.py", "label": "class Foo"}]},
        cfg, store, retained,
    )
    assert len(retained) == 1  # not doubled
    assert "Retained 0 new chunk" not in result  # summary still valid
    assert "[Retained RAG: 1 chunk" in result


def test_retain_returns_error_when_chunk_not_in_store():
    store = _store_with_named_chunks(("/src/a.py", "class Foo"))
    cfg = _config()
    retained: list = []
    result = execute_manage_rag_retention(
        {"retain": [{"file": "/src/a.py", "label": "no_such_label"}]},
        cfg, store, retained,
    )
    assert "Error" in result
    assert "not found" in result.lower()
    assert len(retained) == 0  # nothing was added


def test_retain_returns_error_when_cap_exceeded():
    store = _store_with_named_chunks(("/src/a.py", "class A"), ("/src/a.py", "class B"))
    cfg = _config(max_retained_chunks=1)
    retained: list = [Chunk(content="c", source_file=Path("/src/a.py"), label="class A")]
    result = execute_manage_rag_retention(
        {"retain": [{"file": "/src/a.py", "label": "class B"}]},
        cfg, store, retained,
    )
    assert "Error" in result
    assert "cap" in result.lower()
    assert "1" in result  # cap value mentioned
    assert len(retained) == 1  # unchanged


def test_release_removes_matching_chunks():
    chunk_a = Chunk(content="ca", source_file=Path("/src/a.py"), label="class A")
    chunk_b = Chunk(content="cb", source_file=Path("/src/a.py"), label="class B")
    store = _store_with_named_chunks(("/src/a.py", "class A"), ("/src/a.py", "class B"))
    cfg = _config()
    retained = [chunk_a, chunk_b]
    result = execute_manage_rag_retention(
        {"release": [{"file": "/src/a.py", "label": "class A"}]},
        cfg, store, retained,
    )
    assert len(retained) == 1
    assert retained[0].label == "class B"
    assert "Released 1 chunk" in result


def test_release_ignores_unknown_keys():
    chunk_a = Chunk(content="ca", source_file=Path("/src/a.py"), label="class A")
    store = _store_with_named_chunks(("/src/a.py", "class A"))
    cfg = _config()
    retained = [chunk_a]
    result = execute_manage_rag_retention(
        {"release": [{"file": "/src/nonexistent.py", "label": "ghost"}]},
        cfg, store, retained,
    )
    assert len(retained) == 1  # nothing removed
    assert "Released 0" not in result or "[Retained RAG: 1" in result


def test_combined_release_then_retain_swap_within_cap():
    chunk_a = Chunk(content="ca", source_file=Path("/src/a.py"), label="class A")
    store = _store_with_named_chunks(("/src/a.py", "class A"), ("/src/a.py", "class B"))
    cfg = _config(max_retained_chunks=1)
    retained = [chunk_a]
